...
Maintenance notes
- Do not remove or dramatically restructure `src/GeoGebra.jl` without:
  - Adding a migration/compatibility shim that provides the same public API
  - Updating tests and examples that depend on the bridge
- If you add a new minimal `IJulia` helper module, keep it in `src/comm/` or
  `src/ijulia_helpers.jl` and document usage here.

Next steps (recommended)
- Add an `examples/ijulia_example.jl` demonstrating the `IJulia` comm usage
  (kernel-id helper + inject example).  
- Add a short `test/runtests.jl` that verifies `request()` against a local
  Python bridge process in CI/dev.

If you want, I can add the example file and a small test now.
GeoGebra.jl
===========
