...
# Current active socket (Ref so it can be mutated safely). Used to interrupt
# blocking reads when stopping the client.
...
# Zero-arg overload: parse CLI/ENV and start client with inferred host/port
function start_oob_client()
    ...
    return start_oob_client(host, port)
end
